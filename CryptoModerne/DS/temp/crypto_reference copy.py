...
# =============================================================================
# BLOC 21 — UTILITAIRES : CONVERSION int ↔ bytes ↔ hex
# =============================================================================

# --- int → bytes ---
def int_to_bytes(n: int, longueur: int = None) -> bytes:
    if longueur is None:
        longueur = (n.bit_length() + 7) // 8
    return n.to_bytes(longueur, byteorder='big')
...
n = 123456789
b = int_to_bytes(n)
print(f"int→bytes: {b.hex()}")
print(f"bytes→int: {bytes_to_int(b)}")

# XOR de deux tableaux d'octets (même longueur)
def xor_bytes(a: bytes, b: bytes) -> bytes:
    return bytes(x ^ y for x, y in zip(a, b))

# XOR avec extension cyclique (key stream)
def xor_stream(message: bytes, cle: bytes) -> bytes:
    return bytes(message[i] ^ cle[i % len(cle)] for i in range(len(message)))

print("XOR:", xor_bytes(b'\xAB\xCD', b'\x12\x34').hex())


# =============================================================================
# BLOC 22 — ALGORITHME D'EUCLIDE ÉTENDU (Bézout)
# =============================================================================

def euclide_etendu(a: int, b: int) -> tuple:
    """Retourne (gcd, u, v) tels que a*u + b*v = gcd(a,b)"""
    if b == 0:
        return a, 1, 0
    g, u, v = euclide_etendu(b, a % b)
    return g, v, u - (a // b) * v

# Inverse modulaire via Bézout (équivalent à pow(a,-1,n))
def inverse_bezout(a: int, n: int) -> int:
    g, u, _ = euclide_etendu(a, n)
    if g != 1:
        raise ValueError(f"Pas d'inverse : pgcd({a},{n}) = {g}")
    return u % n

# Exemple
g, u, v = euclide_etendu(35, 15)
print(f"pgcd(35,15)={g}, 35*{u}+15*{v}={35*u+15*v}")
print(f"Inverse de 3 mod 11 = {inverse_bezout(3, 11)}")  # 4


# =============================================================================
# BLOC 23 — TEST DE PRIMALITÉ : MILLER-RABIN
# =============================================================================

import random

def miller_rabin(n: int, k: int = 40) -> bool:
    """Test probabiliste de primalité (k itérations)."""
    if n < 2:  return False
    if n == 2: return True
...
# Générer un grand premier aléatoire
def generer_premier(bits: int) -> int:
    while True:
        n = random.getrandbits(bits) | (1 << bits - 1) | 1  # impair, bit haut à 1
        if miller_rabin(n):
            return n

print(f"1234567891 premier ? {miller_rabin(1234567891)}")
p_test = generer_premier(512)
print(f"Premier 512 bits généré : {p_test.bit_length()} bits")


# =============================================================================
# BLOC 24 — THÉORÈME CHINOIS DES RESTES (CRT)
# =============================================================================

def crt(restes: list, modulos: list) -> int:
    """
    Résoudre le système :
      x ≡ restes[i] (mod modulos[i])  pour tout i
    Les modulos doivent être premiers entre eux deux à deux.
    Retourne la solution unique modulo prod(modulos).
    """
    M = 1
    for m in modulos:
        M *= m
    ...
    x = 0
    for r, m in zip(restes, modulos):
        Mi = M // m
        x += r * Mi * pow(Mi, -1, m)

    return x % M

# CRT pour deux congruences (formule directe)
def crt_deux(a1: int, m1: int, a2: int, m2: int) -> tuple:
    """Retourne (x, M) où x ≡ a1 (mod m1) et x ≡ a2 (mod m2), M = m1*m2."""
    g, u, v = euclide_etendu(m1, m2)
    if (a2 - a1) % g != 0:
        return None, None  # pas de solution
    lcm = m1 * m2 // g
    x = (a1 + m1 * ((a2 - a1) // g * u)) % lcm
    return x, lcm

# Exemple : x ≡ 2 (mod 3) et x ≡ 3 (mod 5) → x ≡ 8 (mod 15)
print("CRT:", crt([2, 3, 2], [3, 5, 7]))  # 23


# =============================================================================
# BLOC 25 — POHLIG-HELLMAN
# =============================================================================

def pohlig_hellman(g: int, b: int, p: int, ordre: int, facteurs: list) -> int:
    """
    Résoudre g^x ≡ b (mod p) quand l'ordre du groupe est friable.
    facteurs = liste de (pi, ei) tels que ordre = prod(pi^ei)
    """
    restes = []
    modulos = []

    for pi, ei in facteurs:
        qi = pi ** ei
        gi = pow(g, ordre // qi, p)   # générateur d'ordre pi^ei
        xi = 0
        gamma = pow(gi, pi ** (ei - 1), p)

        for k in range(ei):
            # Calculer h = (g^{-xi} * b)^{ordre / pi^{k+1}} mod p
            h_exp = ordre // (pi ** (k + 1))
            h = pow(pow(g, xi, p) * pow(b, 1, p) % p, 1, p)
            # Méthode classique : BSGS ou brute force pour petit pi
            h = pow(pow(g, ordre - xi, p) * b % p, h_exp, p)
            # Chercher dk tel que gamma^dk ≡ h (mod p)
            dk = 0
            gamma_k = 1
            for j in range(pi):
                if gamma_k == h:
    ...
    return crt(restes, modulos)

# Exemple simplifié : log discret avec ordre friable
# p = 23, g = 2, ordre de g = 11 (premier → BSGS suffit)
# Pohlig-Hellman utile quand ordre = 2^a * 3^b * 5^c ...


# =============================================================================
# BLOC 26 — ATTAQUES RSA CLASSIQUES
# =============================================================================

import math

# --- Attaque module commun (Common Modulus Attack) ---
# Si même n, deux paires (e1, c1) et (e2, c2) avec pgcd(e1,e2)=1
def attaque_module_commun(n: int, e1: int, c1: int, e2: int, c2: int) -> int:
    """
    Retrouver m si m est chiffré deux fois avec le même n mais e1 et e2 premiers entre eux.
    c1 = m^e1 mod n,  c2 = m^e2 mod n
    """
    g, u, v = euclide_etendu(e1, e2)
    assert g == 1, "e1 et e2 doivent être premiers entre eux"
    # m = c1^u * c2^v mod n (u ou v peut être négatif)
    if u < 0:
        c1_inv = pow(c1, -1, n)
        return pow(c1_inv, -u, n) * pow(c2, v, n) % n
    else:
        return pow(c1, u, n) * pow(c2, -v, n) % n  # v < 0 dans ce cas

# --- Attaque petit exposant public (e=3, même message à 3 destinataires) ---
def attaque_petit_exposant(c_list: list, n_list: list, e: int = 3) -> int:
    """
    Si m^e < n1*n2*...*ne, retrouver m par CRT puis racine e-ième entière.
    c_list[i] = m^e mod n_list[i]
    """
    M = crt(c_list, n_list)
    # Racine e-ième entière de M
    m = round(M ** (1 / e))
    # Vérifier et ajuster
    for delta in range(-2, 3):
        ...
        if candidate ** e == M:
            return candidate
    # Pour grands entiers : Newton-Raphson
    return iroot(M, e)

def iroot(n: int, k: int) -> int:
    """Racine k-ième entière de n (Newton-Raphson)."""
    if n == 0: return 0
    x = int(n ** (1 / k))
    while True:
        x1 = ((k - 1) * x + n // x ** (k - 1)) // k
...
# --- Factorisation RSA si p ≈ q (Fermat) ---
def fermat_factoriser(n: int) -> tuple:
    """
    Factoriser n = p*q si p et q sont proches.
    Rapide si |p-q| est petit.
    """
    a = math.isqrt(n) + 1
    b2 = a * a - n
    while True:
...
# --- Attaque de Wiener (petit exposant privé d) ---
def convergents(n: int, d: int) -> list:
    """Fractions continues de n/d."""
    result = []
    while d:
        result.append(n // d)
        n, d = d, n % d
...
def wiener(e: int, n: int):
    """
    Si d < n^{1/4}/3, retrouver d via fractions continues.
    Retourne d ou None.
    """
    cf = convergents(e, n)
    ...
    for i in range(1, len(cf)):
        ...
        d_test = k_test
        if d_test == 0:
            h_prev, k_prev, h_curr, k_curr = h_curr, k_curr, cf[i]*h_curr+h_prev, cf[i]*k_curr+k_prev
            continue
        # Tester si phi = (e*d - 1) / k donne un phi entier valide
        if (e * d_test - 1) % h_curr == 0:
            phi = (e * d_test - 1) // h_curr
            # n - phi + 1 = p + q, discriminant = (p+q)^2 - 4n
            b = n - phi + 1
            disc = b * b - 4 * n
            if disc > 0:
                sq = math.isqrt(disc)
        ...
        h_prev, k_prev, h_curr, k_curr = (
            h_curr, k_curr,
            cf[i] * h_curr + h_prev, cf[i] * k_curr + k_prev
        )
    return None

# --- Exemple : factorisation de Fermat ---
# p et q proches
p_f = generer_premier(256)
q_f = p_f + 2  # proches (pas nécessairement premiers, juste pour l'exemple)
# En pratique on utilise de vrais premiers proches


# =============================================================================
# BLOC 27 — PKI / CERTIFICATS X.509 (lecture)
# =============================================================================

from Cryptodome.PublicKey import RSA

# --- Lire un certificat PEM (clé publique) ---
def lire_cle_publique_pem(chemin: str):
    with open(chemin, 'rb') as f:
        return RSA.import_key(f.read())

# --- Extraire n et e depuis une clé publique ---
def extraire_parametres_rsa(cle_pub) -> tuple:
    return cle_pub.n, cle_pub.e

# --- Construire clé RSA depuis n, e (clé publique uniquement) ---
def construire_cle_publique(n: int, e: int):
    return RSA.construct((n, e))

# --- Construire clé RSA complète depuis n, e, d, p, q ---
def construire_cle_privee(n: int, e: int, d: int, p: int, q: int):
    return RSA.construct((n, e, d, p, q))

# --- Vérifier cohérence d'une clé RSA ---
def verifier_cle_rsa(n: int, e: int, d: int, p: int, q: int) -> bool:
    checks = [
        n == p * q,
        (e * d) % ((p - 1) * (q - 1)) == 1,
        miller_rabin(p),
...
# =============================================================================
# BLOC 28 — ORACLE DE PADDING / ATTAQUE CBC
# =============================================================================

# --- Simulation d'un oracle de padding (pour comprendre) ---
def padding_oracle_demo(cle: bytes, iv: bytes, chiffre: bytes) -> bool:
    """Retourne True si le déchiffrement a un padding valide."""
    from Cryptodome.Cipher import AES
    from Cryptodome.Util.Padding import unpad
    cipher = AES.new(cle, AES.MODE_CBC, iv)
    try:
        unpad(cipher.decrypt(chiffre), 16)
        return True
    except ValueError:
        return False

# Principe de l'attaque CBC bit-flipping :
# Si on modifie le bloc i du chiffré, le bloc i+1 déchiffré est modifié XOR
# C'[i][j] ^= (original_byte ^ target_byte) pour modifier P'[i+1][j]
def cbc_bit_flip(chiffre: bytes, bloc: int, position: int,
                  original: int, cible: int) -> bytes:
    """Modifier l'octet `position` du bloc `bloc+1` du plaintext."""
    c = bytearray(chiffre)
    c[bloc * 16 + position] ^= original ^ cible
    return bytes(c)


# =============================================================================
# BLOC 29 — RÉSUMÉ DES FORMULES MATHÉMATIQUES CLÉS
# =============================================================================

"""
THÉORÈME DE FERMAT (p premier) :
  a^{p-1} ≡ 1 (mod p)   pour tout a non divisible par p
  → Utile pour vérifier si un nombre est premier (test de Fermat simple)

THÉORÈME D'EULER :
  a^{phi(n)} ≡ 1 (mod n)   si pgcd(a, n) = 1
  phi(n) = n * prod(1 - 1/p) pour p | n
...
RSA — RELATIONS CLÉS :
  ...
  m = c^d mod n = (m^e)^d = m^{ed} = m^{1 + k*phi(n)} = m  (Euler)

DSA — GÉNÉRATION signature (gamma, delta) :
...
DSA — RÉUTILISATION NONCE (même k → même gamma) :
  k = (H(m1) - H(m2)) * (delta1 - delta2)^{-1} mod q
  a = (delta1*k - H(m1)) * gamma^{-1} mod q

DIFFIE-HELLMAN :
  Paramètres publics : p, g
  Alice : x secret, A = g^x mod p
  Bob   : y secret, B = g^y mod p
  Secret : K = g^{xy} mod p

ELGAMAL :
  Clé secrète x, publique b = a^x mod p
  Chiffrer m avec k : c1 = a^k mod p, c2 = m * b^k mod p
  Déchiffrer : s = c1^x mod p, m = c2 * s^{-1} mod p

BSGS (Baby-Step Giant-Step) :
  Chercher x : g^x ≡ b (mod p)
  m = ceil(sqrt(p))
  Stocker g^j pour j=0..m-1
  Parcourir b*(g^{-m})^i jusqu'à collision
  x = i*m + j

POHLIG-HELLMAN :
  Applicable si ordre du groupe est friable : n = p1^e1 * ... * pk^ek
  Résoudre x mod pi^ei pour chaque facteur (BSGS ou brute force)
  Recombiner par CRT

CRT :
  x ≡ ai (mod mi) pour i=1..k, avec mi premiers entre eux deux à deux
  M = m1*m2*...*mk
  x = sum(ai * Mi * Mi^{-1} mod mi) mod M  avec Mi = M/mi

ATTAQUE MODULE COMMUN :
  c1 = m^e1 mod n, c2 = m^e2 mod n, pgcd(e1,e2)=1
  m = c1^u * c2^v mod n  avec e1*u + e2*v = 1 (Bézout)

ATTAQUE PETIT EXPOSANT (e=3) :
  Si m^3 < n1*n2*n3 : CRT donne m^3, puis racine cubique entière

WIENER : si d < n^{1/4}/3 → retrouver d par fractions continues de e/n
"""


# =============================================================================
# BLOC 30 — AIDE-MÉMOIRE RAPIDE : QUEL OUTIL POUR QUEL PROBLÈME
# =============================================================================

"""
PROBLÈME                          → OUTIL / ALGORITHME

Hacher un message                 → SHA256.new(msg).hexdigest()
Authentifier (intégrité + secret) → HMAC
Dériver clé depuis mot de passe   → PBKDF2
Chiffrement symétrique sûr        → AES-GCM (authentifié) ou AES-CBC + HMAC
Chiffrement asymétrique           → RSA-OAEP (pour petits messages)
Chiffrement long message          → Hybride : RSA-OAEP (clé) + AES-CBC (data)
Signature numérique               → RSA-PSS ou DSA
Échange de clé                    → Diffie-Hellman
Chiffrement probabiliste sym.     → ElGamal
Log discret (p ~10^6)             → Baby-Step Giant-Step
Log discret (ordre friable)       → Pohlig-Hellman + CRT
Factoriser n si p≈q               → Fermat
Petit d RSA                       → Wiener (fractions continues)
Même n, deux exposants            → Common Modulus Attack
Même e=3, trois destinataires     → Petit exposant + CRT
Même k DSA deux fois              → Nonce reuse → retrouver clé privée
Résoudre ax ≡ b (mod n)          → Euclide étendu : g|b ? g solutions : 0
Test de primalité                 → Miller-Rabin
Générer un grand premier          → Miller-Rabin sur random.getrandbits
Inverse modulaire                 → pow(a, -1, n) ou Euclide étendu
Exponentiation modulaire          → pow(base, exp, mod)
"""
...
9. pow(a, -1, n) nécessite Python 3.8+ et pgcd(a,n)=1

10. int_to_bytes(0) renvoie b'' -- toujours préciser la longueur si besoin

11. Dans CRT, les modulos doivent être PREMIERS ENTRE EUX (pas seulement premiers)

12. Wiener ne fonctionne que si d < n^{1/4}/3 -- d grand → pas d'attaque

13. CBC bit-flipping corrompt le bloc modifié (déchiffrement ≠ plaintext original)

14. ElGamal est malléable : si on connaît (c1,c2) pour m, (c1, c2*k mod p) chiffre m*k
"""
