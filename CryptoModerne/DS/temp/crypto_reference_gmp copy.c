...
/* =============================================================================
 * BLOC 18 — ALGORITHME D'EUCLIDE ÉTENDU (BÉZOUT)
 * =============================================================================
 *
 * Calcule g, u, v tels que a*u + b*v = g = pgcd(a, b)
 * (mpz_gcdext fait exactement ça)
 *
 * Applications :
 *   - Inverse modulaire : si pgcd(a,n)=1 → a^{-1} mod n = u mod n
 *   - Résolution de ax ≡ b (mod n)
 *   - CRT pour deux congruences
 */

void euclide_etendu_demo(void) {
    mpz_t a, b, g, u, v;
    mpz_init_set_ui(a, 35);
    mpz_init_set_ui(b, 15);
    mpz_init(g); mpz_init(u); mpz_init(v);

    /* g = pgcd(a,b), et a*u + b*v = g */
    mpz_gcdext(g, u, v, a, b);

    gmp_printf("pgcd(35, 15) = %Zd\n", g);
    gmp_printf("35*u + 15*v = %Zd,  u=%Zd,  v=%Zd\n", g, u, v);

    /* Inverse de 3 mod 11 via Bézout */
    mpz_set_ui(a, 3);
    mpz_set_ui(b, 11);
    mpz_gcdext(g, u, v, a, b);   /* 3*u + 11*v = 1 */
    mpz_mod(u, u, b);             /* u mod 11 = inverse de 3 */
    gmp_printf("Inverse de 3 mod 11 = %Zd\n", u);  /* 4 */

    mpz_clear(a); mpz_clear(b); mpz_clear(g); mpz_clear(u); mpz_clear(v);
}

/* Wrapper : retourne u = a^{-1} mod n via Bézout */
void inverse_bezout(mpz_t result, const mpz_t a, const mpz_t n) {
    mpz_t g, u, v;
    mpz_init(g); mpz_init(u); mpz_init(v);
    mpz_gcdext(g, u, v, a, n);
    if (mpz_cmp_ui(g, 1) != 0) {
        fprintf(stderr, "Pas d'inverse : pgcd != 1\n");
        exit(1);
    }
    mpz_mod(result, u, n);
...
/* =============================================================================
 * BLOC 19 — TEST DE PRIMALITÉ : MILLER-RABIN
 * =============================================================================
 *
 * GMP fournit mpz_probab_prime_p(p, reps)
 *   retourne 0 = composé, 1 = probablement premier, 2 = sûrement premier
 *
 * Pour les TP : utiliser directement mpz_probab_prime_p
 */

void miller_rabin_demo(void) {
    mpz_t n;
    mpz_init_set_str(n, "1234567891", 10);

    int r = mpz_probab_prime_p(n, 25);   /* 25 itérations de Miller-Rabin */
    if (r == 2)
        printf("1234567891 est premier (certifié)\n");
    else if (r == 1)
        printf("1234567891 est probablement premier\n");
    else
        printf("1234567891 est composé\n");

    /* Test de petits premiers connus */
    unsigned long premiers[] = {2, 3, 5, 7, 11, 13, 1009};
    for (int i = 0; i < 7; i++) {
        mpz_set_ui(n, premiers[i]);
        r = mpz_probab_prime_p(n, 25);
        printf("%lu : %s\n", premiers[i], r ? "premier" : "composé");
    }

    mpz_clear(n);
}

/* Générer un premier aléatoire de `bits` bits via GMP */
void generer_premier_aleatoire(mpz_t p, unsigned int bits, gmp_randstate_t rng) {
    mpz_t tmp;
    mpz_init(tmp);
    do {
        mpz_urandomb(p, rng, bits);           /* aléatoire sur bits bits */
        mpz_setbit(p, bits - 1);              /* forcer le bit haut */
        mpz_setbit(p, 0);                     /* forcer impair */
    } while (!mpz_probab_prime_p(p, 25));
    mpz_clear(tmp);
}

void exemple_generer_premier(void) {
    gmp_randstate_t rng;
    ...
    mpz_t p;
    mpz_init(p);
    generer_premier_aleatoire(p, 64, rng);
    gmp_printf("Premier 64 bits généré : %Zd\n", p);
...
/* =============================================================================
 * BLOC 20 — CRT GÉNÉRALISÉ (Théorème Chinois des Restes)
 * =============================================================================
 *
 * Résoudre :
 *   x ≡ a1 (mod m1)
 ...
 *   ...
 *   x ≡ ak (mod mk)
 * avec les mi premiers entre eux deux à deux.
 *
 * Formule (deux à deux) :
 *   M12 = m1 * m2
 *   x = a1 + m1 * ((a2 - a1) * m1^{-1} mod m2)  (mod M12)
 */

void crt_generalise(mpz_t x,
                    const mpz_t *a, const mpz_t *m, int k) {
    mpz_t M, tmp, inv, diff, partial;
    mpz_init_set(x, a[0]);
    mpz_init_set(M, m[0]);
    mpz_init(tmp); mpz_init(inv); mpz_init(diff); mpz_init(partial);

    for (int i = 1; i < k; i++) {
        /* x += M * ((a[i] - x) * M^{-1} mod m[i]) */
        mpz_sub(diff, a[i], x);
        mpz_mod(diff, diff, m[i]);
        mpz_invert(inv, M, m[i]);
        mpz_mul(partial, diff, inv);
        mpz_mod(partial, partial, m[i]);
        mpz_mul(tmp, M, partial);
        mpz_add(x, x, tmp);
        mpz_mul(M, M, m[i]);
        mpz_mod(x, x, M);
    }

    mpz_clear(M); mpz_clear(tmp); mpz_clear(inv);
    mpz_clear(diff); mpz_clear(partial);
}

void exemple_crt(void) {
    /* x ≡ 2 (mod 3), x ≡ 3 (mod 5), x ≡ 2 (mod 7) → x ≡ 23 (mod 105) */
    mpz_t a[3], m[3], x;
    mpz_init_set_ui(a[0], 2); mpz_init_set_ui(m[0], 3);
    mpz_init_set_ui(a[1], 3); mpz_init_set_ui(m[1], 5);
    mpz_init_set_ui(a[2], 2); mpz_init_set_ui(m[2], 7);
    mpz_init(x);

    crt_generalise(x, (const mpz_t*)a, (const mpz_t*)m, 3);
    gmp_printf("CRT : x = %Zd (doit valoir 23)\n", x);

    for (int i = 0; i < 3; i++) { mpz_clear(a[i]); mpz_clear(m[i]); }
    mpz_clear(x);
}


/* =============================================================================
 * BLOC 21 — POHLIG-HELLMAN COMPLET
 * =============================================================================
 *
 * Résoudre g^x ≡ beta (mod p) quand l'ordre n du groupe est friable.
 * n = p1^e1 * p2^e2 * ... * pk^ek avec tous pi petits.
 *
 * Pour chaque facteur pi^ei :
 *   gi = g^{n/pi^ei} mod p            (générateur d'ordre pi^ei)
 *   Calculer xi mod pi^ei par BSGS ou brute force
 *   Stocker xi, pi^ei
 * Recombiner par CRT.
 *
 * Ici : implémentation pour pi petit (brute force par pi pour chaque chiffre)
 */

/* Chercher x tel que base^x ≡ target (mod p), x dans [0, ordre) — brute force */
long log_brute(const mpz_t base, const mpz_t target, const mpz_t p, long ordre) {
    mpz_t val;
    mpz_init_set_ui(val, 1);
    for (long x = 0; x < ordre; x++) {
        if (mpz_cmp(val, target) == 0) {
            mpz_clear(val);
            return x;
        }
        mpz_mul(val, val, base);
        mpz_mod(val, val, p);
    }
    mpz_clear(val);
    return -1;
...
/*
 * Pohlig-Hellman pour un seul facteur premier pi avec multiplicité ei.
 * Retourne xi tel que g^xi ≡ beta (mod p) (mod pi^ei)
 */
long pohlig_hellman_facteur(const mpz_t g, const mpz_t beta, const mpz_t p,
                             long n_ordre, long pi, int ei) {
    mpz_t gi, gamma, h, g_neg_xi, tmp;
    mpz_init(gi); mpz_init(gamma); mpz_init(h);
    mpz_init(g_neg_xi); mpz_init(tmp);

    /* gi = g^{n / pi^ei} mod p */
    long qi = 1;
    for (int j = 0; j < ei; j++) qi *= pi;
    mpz_powm_ui(gi, g, (unsigned long)(n_ordre / qi), p);

    /* gamma = gi^{pi^{ei-1}} (générateur d'ordre pi) */
    long pi_ei_1 = 1;
    for (int j = 0; j < ei - 1; j++) pi_ei_1 *= pi;
    mpz_powm_ui(gamma, gi, (unsigned long)pi_ei_1, p);

    long xi = 0;
    mpz_set_ui(g_neg_xi, 1);  /* g^{-xi} mod p, commencer à g^0 = 1 */

    for (int k = 0; k < ei; k++) {
        /* h = (g^{-xi} * beta)^{n / pi^{k+1}} mod p */
        long exp_h = n_ordre;
        for (int j = 0; j <= k; j++) exp_h /= pi;

        mpz_mul(tmp, g_neg_xi, beta);
        mpz_mod(tmp, tmp, p);
        mpz_powm_ui(h, tmp, (unsigned long)exp_h, p);

        /* dk = log_gamma(h) mod pi — brute force car pi petit */
        long dk = log_brute(gamma, h, p, pi);
        if (dk < 0) dk = 0;

        xi = xi + dk * pi_ei_1;  /* xi += dk * pi^{ei-1-k} ... simplification */

        /* Mettre à jour g^{-xi} : multiplier par g^{-dk * pi^k} */
        mpz_t g_dk;
        mpz_init(g_dk);
        long dk_exp = dk;
        for (int j = 0; j < k; j++) dk_exp *= pi;
        mpz_powm_ui(g_dk, g, (unsigned long)dk_exp, p);
        mpz_invert(g_dk, g_dk, p);
        mpz_mul(g_neg_xi, g_neg_xi, g_dk);
        mpz_mod(g_neg_xi, g_neg_xi, p);
        mpz_clear(g_dk);

        pi_ei_1 /= pi;
    }

    mpz_clear(gi); mpz_clear(gamma); mpz_clear(h);
    mpz_clear(g_neg_xi); mpz_clear(tmp);
    return xi % qi;
}

void exemple_pohlig_hellman(void) {
    /*
     * Exemple : p=229, g=6, ordre(g)=228=4*3*19
     * Trouver x tel que 6^x ≡ 115 (mod 229)
     * Facteurs : (2,2), (3,1), (19,1)
     */
    printf("\n=== Pohlig-Hellman ===\n");
    mpz_t p, g, beta;
    mpz_init_set_ui(p, 229);
    mpz_init_set_ui(g, 6);
    mpz_init_set_ui(beta, 115);

    long n_ordre = 228;
    /* Facteurs de 228 = 2^2 * 3 * 19 */
    long pis[]   = {2, 3, 19};
    int  eis[]   = {2, 1, 1};
    int  nb_fact = 3;

    long restes[3];
    mpz_t mod_crt[3], reste_crt[3], x_final;
    for (int i = 0; i < nb_fact; i++) {
        mpz_init(mod_crt[i]); mpz_init(reste_crt[i]);
    }
    mpz_init(x_final);

    for (int i = 0; i < nb_fact; i++) {
        long qi = 1;
        for (int j = 0; j < eis[i]; j++) qi *= pis[i];
        restes[i] = pohlig_hellman_facteur(g, beta, p, n_ordre, pis[i], eis[i]);
        mpz_set_ui(reste_crt[i], (unsigned long)restes[i]);
        mpz_set_ui(mod_crt[i],   (unsigned long)qi);
        printf("x ≡ %ld (mod %ld)\n", restes[i], qi);
    }

    crt_generalise(x_final, (const mpz_t*)reste_crt, (const mpz_t*)mod_crt, nb_fact);
    gmp_printf("x final = %Zd\n", x_final);

    /* Vérification */
    mpz_t verif;
    mpz_init(verif);
    mpz_powm(verif, g, x_final, p);
    gmp_printf("g^x mod p = %Zd  (doit valoir %Zd)\n", verif, beta);

    for (int i = 0; i < nb_fact; i++) { mpz_clear(mod_crt[i]); mpz_clear(reste_crt[i]); }
    mpz_clear(p); mpz_clear(g); mpz_clear(beta);
    mpz_clear(x_final); mpz_clear(verif);
}


/* =============================================================================
 * BLOC 22 — ATTAQUES RSA EN C
 * =============================================================================
 *
 * 1. Factorisation de Fermat (p ≈ q)
 * 2. Attaque module commun (Common Modulus)
 * 3. Calcul de phi(n) si on connaît p et q
 */

/* Fermat : factoriser n si p et q sont proches */
void fermat_factoriser(mpz_t p_out, mpz_t q_out, const mpz_t n) {
    mpz_t a, b2, b, tmp;
    mpz_init(a); mpz_init(b2); mpz_init(b); mpz_init(tmp);
    ...
    while (1) {
        ...
        /* Tester si b2 est un carré parfait */
        mpz_sqrt(b, b2);
        mpz_mul(tmp, b, b);
        if (mpz_cmp(tmp, b2) == 0) {
            /* p = a - b, q = a + b */
    ...
    mpz_clear(a); mpz_clear(b2); mpz_clear(b); mpz_clear(tmp);
}

/* Attaque module commun : retrouver m sachant c1=m^e1 mod n et c2=m^e2 mod n */
void attaque_module_commun(mpz_t m_out,
                            const mpz_t c1, const mpz_t e1,
                            const mpz_t c2, const mpz_t e2,
                            const mpz_t n) {
    mpz_t g, u, v, t1, t2;
    mpz_init(g); mpz_init(u); mpz_init(v);
    mpz_init(t1); mpz_init(t2);

    /* Bézout : e1*u + e2*v = pgcd(e1,e2) */
    mpz_gcdext(g, u, v, e1, e2);  /* g doit valoir 1 */

    /* m = c1^u * c2^v mod n (gérer u ou v négatif) */
    if (mpz_sgn(u) >= 0) {
        mpz_powm(t1, c1, u, n);
    } else {
        mpz_t u_pos, c1_inv;
        mpz_init(u_pos); mpz_init(c1_inv);
        mpz_neg(u_pos, u);
        mpz_invert(c1_inv, c1, n);
        mpz_powm(t1, c1_inv, u_pos, n);
        mpz_clear(u_pos); mpz_clear(c1_inv);
    }
    ...
    if (mpz_sgn(v) >= 0) {
        ...
        mpz_t v_pos, c2_inv;
        mpz_init(v_pos); mpz_init(c2_inv);
        mpz_neg(v_pos, v);
        mpz_invert(c2_inv, c2, n);
        mpz_powm(t2, c2_inv, v_pos, n);
        mpz_clear(v_pos); mpz_clear(c2_inv);
    }

    mpz_mul(m_out, t1, t2);
    mpz_mod(m_out, m_out, n);

    mpz_clear(g); mpz_clear(u); mpz_clear(v);
    mpz_clear(t1); mpz_clear(t2);
}

void exemple_attaques_rsa(void) {
    /* --- Fermat --- */
    printf("\n=== Attaque Fermat ===\n");
    /* n = 11 * 13 = 143, p et q proches */
    mpz_t n, p_f, q_f;
    mpz_init_set_ui(n, 143);
    mpz_init(p_f); mpz_init(q_f);
    fermat_factoriser(p_f, q_f, n);
    gmp_printf("n=143 : p=%Zd, q=%Zd\n", p_f, q_f);  /* 11, 13 */

    /* --- Module commun --- */
    printf("\n=== Attaque module commun ===\n");
    /* m=42, n=143, e1=3, e2=5 (pgcd=1) */
    mpz_t m_orig, e1, e2, c1, c2, m_ret;
    mpz_init_set_ui(m_orig, 42);
    mpz_init_set_ui(e1, 3);
    mpz_init_set_ui(e2, 5);
    mpz_init(c1); mpz_init(c2); mpz_init(m_ret);
    mpz_powm(c1, m_orig, e1, n);
    mpz_powm(c2, m_orig, e2, n);
    attaque_module_commun(m_ret, c1, e1, c2, e2, n);
    gmp_printf("m retrouvé = %Zd (doit valoir 42)\n", m_ret);

    mpz_clear(n); mpz_clear(p_f); mpz_clear(q_f);
    mpz_clear(m_orig); mpz_clear(e1); mpz_clear(e2);
    mpz_clear(c1); mpz_clear(c2); mpz_clear(m_ret);
}


/* =============================================================================
 * BLOC 23 — UTILITAIRES DIVERS
 * =============================================================================
 */

/* Convertir mpz_t → tableau d'octets big-endian */
void mpz_to_bytes(unsigned char **buf, size_t *len, const mpz_t x) {
    *len = (mpz_sizeinbase(x, 2) + 7) / 8;
    *buf = (unsigned char *)malloc(*len);
    mpz_export(*buf, len, 1, 1, 0, 0, x);  /* big-endian */
}

/* Convertir tableau d'octets big-endian → mpz_t */
void bytes_to_mpz(mpz_t x, const unsigned char *buf, size_t len) {
    mpz_import(x, len, 1, 1, 0, 0, buf);  /* big-endian */
}

/* XOR de deux tableaux d'octets */
void xor_bytes(unsigned char *res,
...
void exemple_utilitaires(void) {
    /* mpz_t ↔ bytes */
    mpz_t x;
    mpz_init_set_ui(x, 0xDEADBEEF);

    unsigned char *buf;
    size_t len;
    mpz_to_bytes(&buf, &len, x);
    printf("0xDEADBEEF en bytes (%zu octets) : ", len);
    for (size_t i = 0; i < len; i++) printf("%02X ", buf[i]);
    printf("\n");

    mpz_t y;
    mpz_init(y);
    bytes_to_mpz(y, buf, len);
    gmp_printf("Retour en mpz_t : %Zd\n", y);
...
/* =============================================================================
 * BLOC 24 — RÉCAPITULATIF FORMULES & AIDE-MÉMOIRE RAPIDE
 * =============================================================================
 *
 * QUEL OUTIL POUR QUEL PROBLÈME (version C/GMP) :
 *
 * Log discret (p ~10^6)           → BSGS : bsgs(g, beta, p)
 * Log discret (ordre friable)     → Pohlig-Hellman + CRT
 * Inverse modulaire               → mpz_invert(inv, a, n)
 * Euclide étendu (Bézout)        → mpz_gcdext(g, u, v, a, b)
 * Exponentiation modulaire        → mpz_powm(r, b, e, n)
 * Test de primalité               → mpz_probab_prime_p(p, 25)
 * Générer un premier              → generer_premier_aleatoire(p, bits, rng)
 * Résoudre ax ≡ b (mod n)        → resoudre_lineaire() (BLOC 4)
 * CRT (k congruences)             → crt_generalise() (BLOC 20)
 * RSA : générer n, e, d           → rsa_generer_cles() (BLOC 13)
 * RSA : chiffrer/déchiffrer       → mpz_powm(c, m, e, n) / mpz_powm(m, c, d, n)
 * Fermat (factoriser n si p≈q)    → fermat_factoriser() (BLOC 22)
 * Module commun RSA               → attaque_module_commun() (BLOC 22)
 * DSA signer                      → dsa_signer() (BLOC 11)
 * DSA vérifier                    → dsa_verifier() (BLOC 11)
 * DSA nonce réutilisé             → attaque_nonce_dsa() (BLOC 12)
 * ElGamal chiffrer                → elgamal_chiffrer() (BLOC 10)
 * ElGamal déchiffrer              → elgamal_dechiffrer() (BLOC 10)
 * Diffie-Hellman                  → diffie_hellman() (BLOC 9)
 * Aléatoire GMP                   → gmp_randinit_default + gmp_randseed_ui
 *
 * ORDRE DES PARAMÈTRES DSA.construct (Python) : (y, g, p, q [, x])
 *
 * INITIALISATION ALÉATOIRE GMP :
 *   gmp_randstate_t rng;
 *   gmp_randinit_default(rng);
 *   gmp_randseed_ui(rng, (unsigned long)time(NULL));
 *   mpz_urandomb(x, rng, bits);    // x aléatoire sur `bits` bits
 *   mpz_urandomm(x, rng, n);       // x aléatoire dans [0, n)
 *   gmp_randclear(rng);
 */
...
int main(int argc, char *argv[]) {
    ...
    printf("\n--- BLOC 18 : Euclide étendu ---\n");
    euclide_etendu_demo();

    printf("\n--- BLOC 19 : Miller-Rabin ---\n");
    miller_rabin_demo();

    printf("\n--- BLOC 19 : Générer un premier ---\n");
    exemple_generer_premier();

    printf("\n--- BLOC 20 : CRT généralisé ---\n");
    exemple_crt();

    printf("\n--- BLOC 21 : Pohlig-Hellman ---\n");
    exemple_pohlig_hellman();

    printf("\n--- BLOC 22 : Attaques RSA ---\n");
    exemple_attaques_rsa();

    printf("\n--- BLOC 23 : Utilitaires bytes ---\n");
    exemple_utilitaires();
